...
| Colored toolbar icon option — `resources/icons-streamline/` | **CC BY 4.0** | Free icons from [Streamline](https://streamlinehq.com) (Core flat set) © Streamline / Webalys, recoloured to Open Color green-4 / teal-8 |
| Bundled editor fonts — `resources/fonts/` | **SIL OFL 1.1** (Cascadia Mono, JetBrains Mono, IBM Plex Mono, Iosevka Fixed) / **MIT + Bitstream Vera** (Hack) | Five unmodified monospace families bundled in place of the proprietary Consolas: **Cascadia Mono** © Microsoft (the default *and* the fallback face), **JetBrains Mono** © the JetBrains Mono Project Authors, **IBM Plex Mono** © IBM Corp., **Hack** © Source Foundry Authors (+ Bitstream Vera © Bitstream), **Iosevka Fixed** © Renzhi Li. Cascadia / IBM Plex / Iosevka carry **Reserved Font Names**; see the note below and [`resources/fonts/CREDITS.md`](resources/fonts/CREDITS.md) |
...
| Spell-check engine — Hunspell (`third_party/hunspell/`, `src/hunspell/` subset) | **MPL-1.1** (the elected arm of Hunspell's MPL-1.1 / GPL-2.0-or-later / LGPL-2.1-or-later tri-license) | © László Németh et al. Fetched at build from the [hunspell/hunspell](https://github.com/hunspell/hunspell) tag `v1.7.3` (pinned by SHA256) and built as the `hunspell` static lib from **only** `src/hunspell/` (libhunspell) — never `src/tools/` (its CLI link-depends on GPL readline) — so nothing GPL enters the build. Unmodified, so no MPL modified-file obligation is triggered; the per-file `MPL 1.1/GPL 2.0/LGPL 2.1` license-block headers are kept intact, and the MPL text ships at [`third_party/hunspell/COPYING.MPL`](third_party/hunspell/COPYING.MPL) + [`license.hunspell`](third_party/hunspell/license.hunspell). As Covered Code ships in executable form, the unmodified Hunspell source stays available under the MPL at that tag (see [`NOTICE`](NOTICE)). It is the Linux spell-check backend and the always-available cross-platform fallback (`src/spell_engine.h`) |
| Bundled spell dictionary — `resources/dictionaries/en_US.{aff,dic}` | **SCOWL permissive** (BSD/MIT-style; no copyleft) | American-English Hunspell dictionary derived from [SCOWL](http://wordlist.aspell.net) — word lists © 2000-2018 Kevin Atkinson, affix file © 1993 Geoff Kuenning (BSD). Release `rel-2026.02.25` from [en-wl/wordlist](https://github.com/en-wl/wordlist). The upstream copyright + permission notice ships verbatim at [`resources/dictionaries/en_US_README.txt`](resources/dictionaries/en_US_README.txt) |
