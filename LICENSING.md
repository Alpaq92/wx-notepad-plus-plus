...
| Toolbar icons — `resources/icons/` | **MIT** (+ Apache-2.0, see note) | Tabler © Paweł Kuna, Open Color © Heeyeun Jeong. **Except `wrap-selection.svg`, `print-preview.svg` and `func-node.svg`**, which are original wxNote artwork under **Apache-2.0** — no Tabler glyph exists for those concepts; see `docs/CREDITS.md` |
| Colored toolbar icon option — `resources/icons-solar/` | **CC BY 4.0** (+ Apache-2.0, see note) | Solar Icons (Bold Duotone) © 480 Design, recoloured to Open Color green-8 / green-3. **Except `wrap-selection.svg`, `print-preview.svg`, `func-leaf.svg` and `func-node.svg`**, original wxNote artwork under **Apache-2.0**; see the pack's `CREDITS.md` |
| Colored toolbar icon option — `resources/icons-iconpark/` | **Apache-2.0** | IconPark © ByteDance, recoloured to Open Color teal-7 / lime-5. `wrap-selection.svg`, `print-preview.svg`, `func-leaf.svg` and `func-node.svg` are original wxNote artwork rather than IconPark glyphs — same licence either way, but their provenance is recorded in the pack's `CREDITS.md` |
| Colored toolbar icon option — `resources/icons-streamline/` | **CC BY 4.0** (+ Apache-2.0, see note) | Free icons from [Streamline](https://streamlinehq.com) (Core flat set) © Streamline / Webalys, recoloured to Open Color green-4 / teal-8. **Except `wrap-selection.svg`, `print-preview.svg`, `func-leaf.svg` and `func-node.svg`**, original wxNote artwork under **Apache-2.0** — these four are the generator's `HANDMADE` set, excluded from its manifest check and never regenerated by `tools/generate_streamline_icons.py` |
...
| Ed25519/BLAKE2b verify — Monocypher (`third_party/monocypher/`) | **CC0-1.0** (the elected arm of its CC0-1.0 / BSD-2-Clause dual license) | © Loup Vaillant et al., v4.0.3, vendored (4 files: `monocypher.{c,h}` + the optional `monocypher-ed25519.{c,h}`). C99, no assembly, endian-neutral — the same portability bar `src/hash_algos.h` sets. Used by `src/sig_verify.h` to verify minisign Ed25519 signatures on the plugin catalog (Plugins Admin, `docs/PLUGINS_ADMIN_DESIGN.md`). Vendored rather than self-authored on purpose: the hash_algos doctrine ("pinnable to published vectors") does not transfer to signature *verification*, where a vector-passing implementation can still accept forgeries |
...
> **Reserved Font Name — Cascadia Mono.** Four of the five bundled families are SIL OFL 1.1 (Cascadia
> Mono, JetBrains Mono, IBM Plex Mono, Iosevka Fixed; Hack is MIT + Bitstream Vera), but an OFL licence
> alone does not tell you what is permitted — the Reserved Font Name declaration does, and it differs
> per family. JetBrains Mono's OFL notice declares **no** Reserved Font Name; Cascadia, IBM Plex and
> Iosevka each declare one. Cascadia's notice reads "Copyright (c) 2019 - Present, Microsoft
