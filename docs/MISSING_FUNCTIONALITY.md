...
| **Plugins Admin** (in-app browse/install/update) | **Installed tab shipped**; browse/update deferred | `managePlugins` + the `NibPluginInfo` loader inventory in [main.cpp](../src/main.cpp); state in `<user-data>/plugins.dat` (`wxnSerializePluginState`/`wxnParsePluginState`, 8 round-trip tests) | **Extensions ▸ Manage Plugins…** lists every plugin the loader considered - loaded or not - with its name, version, source and, for anything that did not load, **why**: a plugin that failed used to vanish silently. Plugins can be **enabled/disabled** (including the bundled ones; the GPL bridge is labelled as such because this is where you turn it off) and user-installed ones uninstalled. Every operation is queued and applied on restart, Visual Studio-style, because a loaded plugin is mapped into the process and cannot be deleted or unloaded in place. Disable is keyed on **file name**, not plugin id: the id is only knowable by loading the plugin, which is the thing being avoided. No GUI catalog yet (Available/Updates need a catalog, a fetch and a signed index - see D10); plugins are still dropped in by hand — but that now works on installed builds (plugins load from a user-writable dir), and the signed-catalog core (minisign/Ed25519 verify + parser + validators, 83 tests) is in the tree unused. Browse, install, and update detection all remain gated on there being third-party plugins to list. Status table + open decisions in [PLUGINS_ADMIN_DESIGN.md](PLUGINS_ADMIN_DESIGN.md). |
...
| **Shortcut Mapper** | ✅ **all categories** | saved macros and **Nib plugin commands** now appear (both seeded) + a **Show: All/Menu/Editor/Macros/Plugin commands** category selector — [shortcut_mapper_dialog.h](../src/shortcut_mapper_dialog.h), `seedNibCommandKeymapDefaults` in [main.cpp](../src/main.cpp) | Macros and plugin commands are bindable. Plugin rows are keyed `plugin.<id>` on the stable id the plugin passed to `nib.commands`, never on the positional `NIB_CMD_BASE + i` menu id, so installing or removing a plugin cannot repoint an existing binding at a different command. Seeded after `loadNibPlugins()` (end of the shell frame ctor) rather than beside the macro seeding in `buildMenuBar()`, which runs before any plugin exists. A command registered with no id is not bindable — there is nothing stable to key it to. **Run commands are now a real category too**: wxNote grew user-defined Run commands to populate it. **Run ▸ Run…** gained a **Save…** button that names the command; named commands persist to `runcommands.dat`, list at the bottom of the Run menu (`myID_RUN_ITEM` 62400+, beside saved macros), are renamed/deleted from **Run ▸ Manage Saved Commands…**, and bind under `run.<uid>` - keyed on the uid, so reordering or deleting one never repoints another's shortcut. The `run.` prefix is shared with the static menu commands (`run.execute`, `run.manageSaved`, `run.validateShortcutsXml`), so the category test requires an all-digits uid tail, the same guard saved macros needed |
| **Function List** | partial (extensible) | +11 more built-in languages (markdown/yaml/ini/css/makefile/dockerfile/batch/perl/kotlin/swift/r), name-based detection for extension-less `Makefile`/`Dockerfile`, plus the user overlay `functionList.conf` merged over the built-ins — [main.cpp](../src/main.cpp) `flRules`/`loadFunctionListRules`/`flLangKey` | 24 built-in languages + user-extensible; still `std::regex` (not N++'s GPL `functionList.xml`), so nesting is brace/indent-scanned rather than parsed — **but the scan is no longer regex-blind**. `flCollect` now takes the comment/string map as sorted spans, built by `flProseZones` from the style bytes of the lexer that is *already running* (Lexilla via `TagsOfStyle`, or the Scintillua container styles), so a brace inside a construct no pattern covers — a multi-line raw string, a nested or doc comment, an f-string — is no longer counted as nesting and no longer swallows the rest of the file. It also skips the regex mask pass entirely, and with it the `std::regex_error(error_stack)` that pass throws on a large block comment. The map is preferred, never required: an unstyled container buffer or a lexer exposing no comment/string tags falls back to the old regex zones (the test is the pointer, never emptiness - an empty supplied list is the lexer saying the file genuinely has none). Styles are read in 256 KiB chunks via `SCI_GETSTYLEDTEXT`, not one `GetStyleAt` message per byte as first shipped in 0.18.0: measured on a 15.1 MiB buffer that was **753 ms -> 214 ms** with the styling warm-up excluded from both, and the transient allocation **15.86 MB -> 1.72 MB**, on a path the Function List's debounce timer re-runs after every edit burst |
...
| **Style Configurator** | partial | fg/bg, Bold/Italic/**Underline**, per-style **font name / size / weight**, and a **`genericLangDef`** entry that finally makes Scintillua-lexed (custom/UDL) languages follow the theme, plus **Save As…** and **Revert changes** — `onStyleConfig` / `synthesizeGenericStyles` / `applyStyleFont` / `applyScintilluaStyles` / `userThemeDir` / `reloadThemeLive` in [main.cpp](../src/main.cpp) (the old `main.cpp:9784` anchor was stale) | Underline now renders: bit 4 of Notepad++'s `fontStyle` was parsed and saved all along but never applied, so 167 authored styles across the 28 shipped themes drew plain. Custom-language colours were previously a fixed palette that ignored the theme entirely; the `genericLangDef` block is derived from each theme's own `cpp` styles at load time and written to the theme file only once the user edits one of them, so it works for third-party Notepad++ themes too. `keyword2..8` stay on the built-in palette by design — no Scintillua lexer emits those tags; they are a UDL's own keyword groups, which Notepad++ stores in the UDL file rather than the theme. Per-style font: `fontName` and `fontSize` are Notepad++'s own `WordsStyle` attributes and round-trip to it exactly (0 of 49,507 shipped styles set either, so nothing regresses); `fontWeight` is a **wxNote extension**, and the limitation is in the FORMAT, not the engine - checked, not assumed. Scintilla supports per-style weight outright (`SCI_STYLESETWEIGHT` 2063, with `SC_WEIGHT_NORMAL` 400 / `SEMIBOLD` 600 / `BOLD` 700), and it is present in wx's bundled Scintilla 5.0.0, so weights render correctly here. What cannot carry them is Notepad++'s `WordsStyle` schema: it has `fontName`/`fontStyle`/`fontSize` and no weight of any kind (0 occurrences of `fontWeight` across all 28 shipped themes and `stylers.model.xml`). **Decision: keep the extension.** To be precise about whose format this is: the theme FILES are wxNote's own (Apache-2.0, regenerated, carrying no Notepad++ theme data), and the SCHEMA is Notepad++-compatible by deliberate choice so a third-party N++ theme loads here unmodified. So the format is ours to extend, and `fontWeight` is that extension; Notepad++ ignores attributes it does not know when it READS. What is outside our control is only that Notepad++ drops the attribute when it WRITES. The alternatives were to drop per-style weight entirely - losing a capability the engine genuinely has, to protect one direction of one round trip - or to fake it by collapsing weight onto the bold bit, which would silently change what the user chose. Instead the attribute is written only onto styles that actually use one, so a theme that never sets a weight is byte-comparable with Notepad++'s own output, and a theme round-tripped THROUGH Notepad++ keeps the bold flag and loses only the exact weight. That loss is the accepted cost and is not fixable from this side. An explicit weight overrides the bold bit at render time. **Save As…** copies the whole active theme into `<userDataDir>/themes/<name>.xml` and folds the pending edits into the copy, which is also the only theme folder an installed build can write — `<exeDir>/themes` is not user-writable, and the light-mode `Default` maps to `stylers.model.xml` there, so Save As is the supported way to keep edits to a bundled theme. A user theme shadows a bundled one of the same name and is listed once, not twice - **known gap: there is no in-app way to undo that shadowing**, since the dialog has no delete-user-theme affordance; recovering the bundled theme means deleting the file from `<user-data>/themes` by hand. Not an engine limit, just unbuilt. A failed save now SAYS so and names Save As... as the way out: the light-mode `Default` targets `<exeDir>/stylers.model.xml`, which an installed build cannot write, and that case previously produced no message at all - the edits were simply discarded in silence. **Revert changes** re-reads the theme from disk mid-session (what Cancel does on close) without re-pinning the theme name, so a window still following dark/light mode keeps following it. The dialog is now sized by its sizer rather than a fixed 860×470, which was clipping the font-name combo in Polish. Still no keyword-set editor and no global overrides. Per-style **strike/overline are unreachable** — no upstream Scintilla has them (Notepad4 gets them from a private fork, on message ids upstream assigns to `SCI_MARKERSETFORE`/`BACK`) |
