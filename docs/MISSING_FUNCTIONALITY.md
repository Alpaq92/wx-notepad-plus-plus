...
| **Calltips** | partial (**project-wide**) | `funcCallTip`/`callTipSigs` + `wxnIndexSigs`/`stepSigIndex` in [main.cpp](../src/main.cpp); 10 tests in `tests/funclist_selftest.cpp` | Signatures now come from the open document FIRST and then from a **workspace signature index**, so a function defined in another file of the project gets a hint too - previously you got one only for a function already on screen, which is the case you least need it for. The index is phase two of the Quick Open crawl: once that file list exists, the same 12 ms timer slice reads those files and records every `identifier(...)` signature. Bounded four ways because it reads a whole project - only extensions that map to a Function List language (the crawler's `wxnSkipExt` is a blocklist for binaries; this is the allowlist for "parentheses mean a call"), a 1 MiB per-file cap, 8 overloads per name, 40k names total. The extraction itself (`wxnSigAt`) is shared with the keystroke path so the two cannot drift. **Still missing, and this is the big one:** nothing for stdlib or third-party calls - `std::vector::push_back` has no signature here, because nothing in the workspace defines it. That needs either shipped API data (a maintenance treadmill, and weaker licensing ground than the keyword lists) or an LSP client. **Also:** the index is built once per workspace and is not invalidated on save, so a signature added after indexing is not seen until the workspace is re-opened. |
