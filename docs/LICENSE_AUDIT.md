...
- `src/main.cpp:429` — explicitly states the Function List regex rules were "distilled from N++'s XML and simplified to portable std::regex" because N++'s own PCRE/Boost-recursion patterns *cannot* be run as-is — i.e., this comment documents that verbatim reuse was impossible and a rewrite was done instead.
- `src/npp_menu.h:4` — documents that the menu structure is a "1:1 reproduction of IDR_M30_MENU from PowerEditor/src/Notepad_plus.rc" in terms of layout/IDs/labels — this refers to reproducing menu *data* (labels, mnemonics, `IDM_*` command IDs), not C++ implementation; the actual menu-building code is an original, data-driven `wxMenuBar` builder.
...
| Inno Setup / ISCC.exe | preinstalled on CI runner | Freeware, own license (not GPL/LGPL) | **Build-tool-only.** Compiles the installer script into a `Setup.exe` wrapper; contributes no files to the shipped `[Files]` list (which contains only this project's own build outputs). |
...
All four project names appear **only** in `src/main.cpp` (four locations total) — none appear in `include/`, `packages/`, or any file under `docs/`.
...
Ranked by severity; **nothing here rises to a confirmed legal problem** — the most notable item is a documentation-consistency nit.

1. **[Should document better, not a legal concern] `resources/icons/` has no in-directory `CREDITS.md`.** Its two colored sibling sets (`icons-solar/`, `icons-iconpark/`) each have a detailed `CREDITS.md` documenting source, license, and modifications. The default set's MIT/Tabler/Open-Color attribution exists correctly in root `LICENSING.md` and `NOTICE`, so there is no missing or false claim — but the inconsistency between sibling directories is worth fixing for uniformity, especially since the icons are the asset class most likely to be redistributed or forked independently.

2. **[Minor wording nit, not a legal concern] `src/main.cpp:6088-6089`'s comment about theme parsing** ("Parse Notepad++'s real theme XML ... so the editor uses Notepad++'s exact colours") reads, in isolation, as if `stylers.model.xml` contains literal copied N++ data. The file's own embedded header and both root docs correctly clarify it is a regenerated, differently-licensed file — but the in-code comment itself could be reworded to avoid the misleading first impression for future contributors reading the code without also checking the file header.

3. **[No action needed, informational only] The `LangType` enum in `include/npp-compat/` has the least "daylight" from the original N++ header** of anything in the clean-room replacement, since enum member order must match exactly for ABI compatibility. This was flagged by the researcher as worth noting but is explicitly defensible under the project's own stated clean-room rule (facts/IDs may be reproduced; expression may not) — there is no practical way to give a fixed-value enum "creative" alternate content, so this is inherent to the compat layer's purpose rather than a shortfall in execution.
