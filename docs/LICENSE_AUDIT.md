# wxNote — Leftover Code & Dependency License Audit
...
> zlib-licensed open-source NSIS (`installer/windows/wxnote.nsi`); (4) the four reference-project
...
> provenance anymore.
>
> **Third remediation addendum (2026-07-11):** one finding this audit *missed* was later caught and
> fixed: `resources/themes/DansLeRuSH-Dark.xml` (© Franck Albaret) was counted among the "kept upstream
> permissive themes" in Section 2 because its header opens with MIT-style permission text — but that
> text is labeled "[ LEGAL DISCLAIMER ]" and the same header's explicit `Licence :` field, like the
> author's canonical repo (codeberg.org/DansLeRuSH/notepad-plus-plus-dark-theme), declares
> **CC BY-NC-SA 3.0 (NonCommercial)**. The theme was removed from the shipped set on 2026-07-11 and the
> themes rows in `LICENSING.md`, `NOTICE`, and `README.md` were corrected (see `LICENSING.md`'s removal
> note).
...
- License banners were replaced with an SPDX Apache-2.0 / wxNote copyright header explaining the clean-room rationale.
...
- `src/main.cpp:1687` and `:3043` — describe wxNote's own original code in relation to how N++'s equivalent behaves, for context only.
...
| `resources/themes/*.xml` (27 files) | `resources/themes/` | **Mixed** — ~14 files are genuine kept upstream permissive themes with original author headers intact (MIT-style: Fabio Zendhi Nagao, Oren Farhi, Paul Neubauer, Renato Silva, and one unattributed-author-but-licensed file); ~13 files are wxNote-regenerated Apache-2.0 replacements naming their source palette's license (GitHub Primer, Atom One Dark/Light, Nord, Dracula, VS Code — all MIT — plus Zenburn/Obsidian as "canonical palettes, colour values are facts") | Yes — every one of the 27 files was individually opened; every file carries either the original author's license header or the project's own regenerated Apache-2.0 header. None is a silently-copied N++ asset. **Since amended (2026-07-11):** one of the "kept permissive" files, `DansLeRuSH-Dark.xml`, was misread here — its MIT-style text is labeled a "LEGAL DISCLAIMER" and its actual declared license (header `Licence :` line + the author's canonical repo) is CC BY-NC-SA 3.0 (NonCommercial). It has been removed from the shipped set; see the third remediation addendum above. |
| `resources/wxnote.svg` (app icon) | `resources/` | Original work — a green rounded-square plate with a white "N" monogram; not adapted from N++'s actual notepad-with-bent-corner logo | Yes — both `LICENSING.md` and `NOTICE` self-disclose that the icon's styling deliberately "echoes" upstream N++ branding while being original artwork. Git history confirms it was introduced during the de-N++-ification restructuring, not carried over from an N++ tree. |
| `resources/wxnote.ico` | `resources/` | Same asset family as the `.svg` above (binary rasterization) | Implicit — no separate license note, but `resources/app.rc`'s comment confirms it's the Win32 icon-resource rasterization of the same original SVG. |
...
|---|---|---|---|
| wxWidgets | 3.3.1 | wxWindows Library Licence 3.1 (LGPLv2+ with a static/binary-linking exception; permits closed-source use) | Fetched via `FetchContent` from the official GitHub release tarball; built from source with `wxBUILD_SHARED OFF` — **statically linked** into `wxnote.exe`. |
| Scintilla | vendored (via `third_party/scintilla/`, headers used directly; also bundled inside wxWidgets' `wxStyledTextCtrl`) | HPND (Historical Permission Notice and Disclaimer) — MIT/BSD-style permissive, Neil Hodgson | Source vendored in-repo; headers (`Scintilla.h`, `ILexer.h`) used directly by Lexilla and wxnote; compiled in via wx's bundled copy — effectively **statically bundled**. |
| Lexilla | vendored (`third_party/lexilla/`) | HPND — identical license text to Scintilla, Neil Hodgson | Built as a static library `lexilla` from vendored sources; **statically linked** into `wxnote`. |
| wxBorderlessFrame (wxbf) | vendored (`third_party/wxbf/`) | wxWindows Library Licence 3.1 (LGPLv2+ with static-linking exception) | Vendored source, built as static library `wxbf`; **statically linked** into `wxnote` on Windows and Linux only (no macOS backend). |
...
| rpm / rpmbuild | `apt-get install rpm` at build time (Linux) | GPL-2 / LGPL-2 (dual, rpm project) | **Build-tool-only.** `installer/linux/wxnote.spec` has no `%build` step (nothing is compiled by rpmbuild) - it only stages the project's own already-built `build/bin/` output into `%{buildroot}` and serializes that into `.rpm` format; contributes no files of its own to the package payload. |
| flatpak-builder | `apt-get install flatpak flatpak-builder` at build time (Linux) | LGPL-2.1+ | **Build-tool-only.** `installer/linux/io.github.Alpaq92.WxNote.yml` uses `buildsystem: simple` - it copies the project's own already-built `build/bin/` output into the sandbox and installs it under `/app/bin`; no flatpak-builder code is compiled into or shipped inside the resulting `.flatpak` bundle. |
| org.gnome.Platform / org.gnome.Sdk `//46` | Flatpak runtime, installed from Flathub at build time (Linux) | Mixed permissive/LGPL (GNOME runtime component licenses; GTK3 itself LGPL-2.1+) | **Bundled into the shipped `.flatpak`, not build-tool-only** - unlike every other entry in this table, the GNOME Platform runtime *is* part of what end users receive and run (Flatpak's whole model is running the app against a shared runtime the user's `flatpak` also installs) - but it is a dynamically-loaded, separately-versioned system component, not statically linked or vendored source, the same LGPL posture as GTK3 itself elsewhere in this table. |
...
**Linking-model summary:** wxWidgets, Scintilla, Lexilla, and wxBorderlessFrame are all statically linked. GTK3 is a dynamically-linked open-source dependency for the AppImage/.deb/.rpm Linux packages and for a from-source Linux build (LGPL-compliant pattern in all cases). The `.flatpak` package instead depends on the GNOME Platform runtime at install/run time - not statically linked, and not vendored into the bundle's own payload, but genuinely shipped-with (see its table row above) rather than build-tool-only, since Flatpak's whole distribution model is running against a separately-installed shared runtime. Windows system libraries are dynamically linked OS components. Everything else in this table is either build-tooling that never ships, or CI-only infrastructure.
...
- **CI/installer scripts on platforms not exercised in this session** (e.g., actual execution of `installer/macos/build-dmg.sh`, `installer/linux/build-appimage.sh`, `installer/linux/build-rpm.sh`, or `installer/linux/build-flatpak.sh`) were read statically, not run, so runtime-only side effects (e.g., what linuxdeploy or flatpak-builder actually bundle for a given build environment) were inferred from script logic and later confirmed only via CI (see the addendum), not observed directly on a local Linux/macOS machine.
