...
| rpm / rpmbuild | `apt-get install rpm` at build time (Linux) | GPL-2 / LGPL-2 (dual, rpm project) | **Build-tool-only.** `installer/linux/wxnpp.spec` has no `%build` step (nothing is compiled by rpmbuild) - it only stages the project's own already-built `build/bin/` output into `%{buildroot}` and serializes that into `.rpm` format; contributes no files of its own to the package payload. |
| flatpak-builder | `apt-get install flatpak flatpak-builder` at build time (Linux) | LGPL-2.1+ | **Build-tool-only.** `installer/linux/io.github.Alpaq92.WxNotepadPlusPlus.yml` uses `buildsystem: simple` - it copies the project's own already-built `build/bin/` output into the sandbox and installs it under `/app/bin`; no flatpak-builder code is compiled into or shipped inside the resulting `.flatpak` bundle. |
